...
export default function UpdateResultatCameroun() {
  const history = useHistory()
  const { id } = useParams()

  const [ResultatCameroun, setResultatCameroun] = useState(null)
  ...
  useEffect(() => {
    async function fetchData() {
      const [res, err] = await queryApi("ResultatCameroun/" + id)
      if (err) setError(err)
      else {
        setResultatCameroun(res)
  ...
  async function handleSubmit(e) {
    ...
    setShowLoader(true)
    //Update press
    const [, err] = await queryApi("ResultatCameroun/" + id, form, "PUT", true)
      ...
      <Col xs="12" xl="6">
        <Card className="height100">
          <CardHeader>
            ...
            <strong>Resultat Cameroun : </strong>
          </CardHeader>
        </Card>
      </Col>
      <Col xs="12" xl="6">
        <Card>
          <CardHeader>
            <strong> Resultat Cameroun : </strong>Modifier
          </CardHeader>
          <CardBody>
            {formError.visible && (
