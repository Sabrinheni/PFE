...
export default function ShowRdis() {
  ...
  useEffect(() => {
    async function fetchData() {
      const [res, err] = await queryApi("rdis")
      setError(err)
      setRdis(res)
    }
    fetchData()
          ...
          <Card>
            <CardHeader>
              <Form action="" method="post" className="form-horizontal" onSubmit={handleSubmit}>
                ...
                {(currentUser.roles.includes("createRdi") || currentUser.roles.includes("admin")) && (
                  <FormGroup row>
                    <Col md="4" sm="4">
                      <InputGroup className="mt-2">
            ...
            <CardBody>
              ...
              <Row>
                {filtered &&
                  filtered?.length > 0 &&
                  filtered.map(c => <Rdi key={c._id} currentUser={currentUser} rdi={c} setRdis={setRdis} />)}

                {filtered && filtered?.length === 0 && (
                  <Col md="4" sm="4">
                    <Alert color="warning">Pas de rdis trouvés.</Alert>
                  </Col>
                )}
              </Row>
            </CardBody>
